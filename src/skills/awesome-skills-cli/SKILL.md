...
- `list`, `search`, `info`, `add`, `setup`, `update`, and `version`
...
Command behavior:
- `list` outputs plain text with optional `--limit` and `--offset` flags; pagination metadata goes to stderr.
...
- If the binary is missing, tell the user to install it first before relying on `add`, `setup`, or `list`.
